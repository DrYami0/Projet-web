...
class GameC
{
    ...
    // Complete a game and update player stats
    public function completeGame($gid, $winner, $rounds_played = null, $game_state = null, $player1Score = 0, $player2Score = 0) {
        $db = config::getConnexion();
        ...
        try {
            ...
            // Update player stats
            $this->updatePlayerStats($game, $winner, $player1Score, $player2Score);
    ...
    /**
     * Update both players' stats after game completion
     */
    private function updatePlayerStats($game, $winner, $player1Score, $player2Score) {
        $userC = new UserC();
        
        // Determine game type (1, 2, or 3) from the 'game' field
        $gameType = $this->getGameTypeNumber($game['game']);
        ...
        if ($player1id) {
            ...
            $userC->updateGameStats($player1id, $gameType, $player1Score, $player1Won);
        }
        
        // Update player 2 stats (if exists - multiplayer game)
        if ($player2id) {
            $player2Won = ($winner == $player2id);
            $userC->updateGameStats($player2id, $gameType, $player2Score, $player2Won);
        }
    }

    /**
     * Convert game name to game type number
     */
    private function getGameTypeNumber($gameName) {
        $gameTypes = [
            'game1' => 1, 'grammar' => 1, 'grammaire' => 1,
            'game2' => 2, 'vocabulary' => 2, 'vocabulaire' => 2,
            'game3' => 3, 'conjugation' => 3, 'conjugaison' => 3,
        ];
        
        $gameName = strtolower($gameName);
        return $gameTypes[$gameName] ?? 1;
    }
