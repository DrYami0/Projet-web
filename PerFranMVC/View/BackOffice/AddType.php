...
require_once "../../PerFranMVC/Model/Type.php";
    ...
    if ($old['type'] === '') {
        ...
        if ($old['type'] === '' || !in_array($old['type'], $allowedTypes, true)) {
            ...
            try {
                ...
                $type = new Type($old['type'], $old['difficulty']);
                $typeC = new TypeC();
                $typeC->addType($type);
                header('Location: DisplayTypes.php?added=1');
                exit;
            } catch (Exception $e) {
...
<head>
  ...
  <title>PerFran — Ajouter un type</title>
...
<body>
  <header class="MainTitle smallHeader">
    <a href="/projet-web/index.php" class="brand">
      ...
      <div>
        ...
        <p>Remplissez le formulaire pour ajouter un type</p>
      </div>
    </a>
  </header>
  ...
  <main class="container">
    <section class="formWrap tableWrap">
      ...
      <form method="post" action="AddType.php" class="addForm" id="addTypeForm" novalidate>
        <label>
