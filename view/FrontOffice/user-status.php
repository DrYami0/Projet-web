...
$pendingFile  = __DIR__ . '/../../controller/pending/' . $username . '.json';
$approvedFile = __DIR__ . '/../../controller/approved/' . $username . '.json';
$refusedDir   = __DIR__ . '/../../controller/refused/';
...
$createdAt = filemtime($pendingFile);
$remaining = 48 * 3600 - (time() - $createdAt);

if ($remaining <= 0) {
    // Auto-refus
    if (!is_dir($refusedDir)) mkdir($refusedDir, 0777, true);
    rename($pendingFile, $refusedDir . $username . '.json');
    unset($_SESSION['pending_username']);
    ...
    <style>
        * {
        ...
        body::before {
            ...
            width: 100%;
            height: 100%;
            background: url('<?= BASE_URL ?>view/FrontOffice/assets/images/waiting-bg.gif') center center;
            background-size: cover;
            z-index: -2;
        }

        /* Fallback gradient background if GIF doesn't load */
        body::after {
            content: '';
            position: fixed;
            top: 0;
